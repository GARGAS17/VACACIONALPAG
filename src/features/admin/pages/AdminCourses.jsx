            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" placeholder="Nombre del curso" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} required className="flex-1 px-3.5 py-2.5 bg-slate-700/50 border border-slate-700 rounded-xl text-slate-200 text-sm focus:outline-none focus:border-green-500 placeholder-slate-500" />
                  <button 
                    type="button" 
                    onClick={handleAIGenerate}
                    disabled={isGeneratingAI || !formData.title}
                    className="px-3 bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 text-white rounded-xl text-xs font-bold transition-all disabled:opacity-50 shadow-md border-none cursor-pointer flex items-center justify-center gap-1.5"
